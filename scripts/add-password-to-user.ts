 ...
 * Usage: npx tsx scripts/add-password-to-user.ts <email> <password>
 */

import { hash } from 'bcryptjs';
import { dynamoDBUsers } from '../src/lib/dynamodb';
import { maskEmail } from '../src/lib/safe-logger';

async function addPasswordToUser(email: string, password: string) {
  try {
...
if (args.length !== 2) {
  console.error('Usage: npx tsx scripts/add-password-to-user.ts <email> <password>');
  console.error('Example: npx tsx scripts/add-password-to-user.ts user@example.com MyP@ssw0rd');
  process.exit(1);
}
