# PowerShell script to deploy Kinex Fit to AWS
# Full infrastructure setup - Use this for first-time deployment only
# For updates to existing deployment, use update-deployment.ps1 instead
...
Write-Host "🚀 Starting Kinex Fit full deployment to AWS..." -ForegroundColor Green
Write-Host "⚠️  WARNING: This script creates new AWS infrastructure" -ForegroundColor Yellow
Write-Host "   Use 'update-deployment.ps1' for routine updates to existing infrastructure" -ForegroundColor Yellow
Write-Host ""
...
aws iam create-role --role-name SpotterTaskExecutionRole --assume-role-policy-document $TrustPolicy 2>$null
aws iam attach-role-policy --role-name SpotterTaskExecutionRole --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy

# Additional policy for Systems Manager access (for environment variables)
$SsmPolicy = @"
{
    "Version": "2012-10-17",
    "Statement": [
...
aws iam put-role-policy --role-name SpotterTaskExecutionRole --policy-name SsmParameterAccess --policy-document $SsmPolicy

Write-Host "✅ ECS execution role configured" -ForegroundColor Green

Write-Host "📝 Step 6: Creating ECS task role for application..." -ForegroundColor Yellow
aws iam create-role --role-name SpotterTaskRole --assume-role-policy-document $TrustPolicy 2>$null

# Task role policy for DynamoDB, Bedrock, SES, etc.
$TaskPolicy = @"
{
    "Version": "2012-10-17",
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:$Region:$AccountId:table/spotter-*"
            ]
        },
        {
            "Effect": "Allow",
...
aws iam put-role-policy --role-name SpotterTaskRole --policy-name SpotterAppPolicy --policy-document $TaskPolicy
Write-Host "✅ ECS task role configured" -ForegroundColor Green
...
Write-Host "ℹ️  Using existing aws-task-definition.json file" -ForegroundColor Blue

# Check if task definition file exists
if (-not (Test-Path "aws-task-definition.json")) {
    Write-Error "❌ aws-task-definition.json not found. Please ensure it exists in the project root."
    exit 1
}
...
$AlbSgId = (aws ec2 create-security-group --group-name spotter-alb-sg --description "Security group for Kinex Fit ALB" --vpc-id $VpcId --query "GroupId" --output text --region $Region)
...
$EcsSgId = (aws ec2 create-security-group --group-name spotter-ecs-sg --description "Security group for Kinex Fit ECS tasks" --vpc-id $VpcId --query "GroupId" --output text --region $Region)
...
Write-Host "🌐 Your Kinex Fit app is available at: http://$AlbDns" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "3. Set up Route53 DNS record to point to the ALB" -ForegroundColor White
Write-Host "4. Configure SSL certificate using AWS Certificate Manager" -ForegroundColor White
Write-Host "5. Update NEXTAUTH_URL in SSM parameters to use your domain" -ForegroundColor White
Write-Host "6. Configure Stripe webhook endpoint with your domain" -ForegroundColor White
Write-Host ""
Write-Host "📚 For routine updates, use: .\scripts\update-deployment.ps1" -ForegroundColor Cyan
